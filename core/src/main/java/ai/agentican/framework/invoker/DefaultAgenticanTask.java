...
record DefaultAgenticanTask<P, R>(
        Agentican runtime,
        Supplier<Plan> planSource,
        Class<P> paramsType,
        Class<R> outputType) implements AgenticanTask<P, R> {

    DefaultAgenticanTask {

        if (runtime == null)    throw new IllegalArgumentException("Agentican is required");
