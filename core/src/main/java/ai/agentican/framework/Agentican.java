...
public class Agentican implements AutoCloseable {
    ...
    public TaskHandle run(Plan plan, Map<String, String> taskInputs) {
        ...
        return submit((taskId, cancelled) -> {
            ...
            return taskRunner.run(plan, taskId, taskInputs, cancelled);
        });
    }
    ...
    public static class AgenticanBuilder {
        ...
        // Declarative config accumulators (merged into RuntimeConfig at build time)
        ...
        // Live instances (can't live in a RuntimeConfig record)
        ...
        public Agentican build() {
            ...
            // Framework-service defaults
            var hm = hitlManager != null ? hitlManager : new HitlManager(HitlNotifier.logging());
            var ks = knowledgeStore != null ? knowledgeStore : new MemKnowledgeStore();
            var tss = taskStateStore != null ? taskStateStore : new MemTaskStateStore();
            ...
            // 1) LlmClients: build from configs (with decorator + retry), then merge injected instances
            ...
            // 2) Wrap the TaskStateStore with notifier + optional knowledge ingestor
            TaskStateStore notifyingStore = new NotifyingTaskStateStore(tss, tl);
            ...
            if (defaultLlm != null) {
                ...
                notifyingStore = new NotifyingTaskStateStore(notifyingStore, knowledgeIngestor);
            }

            var finalTss = notifyingStore;

            // 3) Toolkit registry: MCP + Composio + user-injected toolkits
            ...
            // 4) Skill registry (override or InMemory) + seed + register from config
            SkillRegistry sr = skillRegistry != null ? skillRegistry : new InMemorySkillRegistry();
            sr.seed();
            ...
            // 6) Agent registry (override or InMemory) + seed + register from config
            AgentRegistry ar = agentRegistry != null ? agentRegistry : new InMemoryAgentRegistry();
            ...
            // 7) Plan registry (override or InMemory) + seed + register from config
            PlanRegistry pr = planRegistry != null ? planRegistry : new InMemoryPlanRegistry();
            pr.seed();
