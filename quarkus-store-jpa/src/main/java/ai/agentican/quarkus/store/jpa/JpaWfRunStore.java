...
public class JpaWfRunStore implements WorkflowRunStore {

    private static final Logger LOG = LoggerFactory.getLogger(JpaWfRunStore.class);
    ...
    private WorkflowRunLog reconstruct(TaskEntity task) {

        Map<String, String> params = readJsonParams(task.paramsJson);

        WorkflowDefinition plan = null;
        ...
        if (task.planSnapshotJson != null && !task.planSnapshotJson.isBlank()) {
            try { plan = readPlan(task.planSnapshotJson); }
                ...
                LOG.warn("Failed to deserialize definition snapshot for task {}: {}", task.id, ex.getMessage());
    ...
    private static WorkflowRunStatus parseStatus(String s) {

        if (s == null) return null;
        try { return WorkflowRunStatus.valueOf(s); }
