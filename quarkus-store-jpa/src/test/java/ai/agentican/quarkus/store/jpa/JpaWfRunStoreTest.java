...
class JpaWfRunStoreTest {
    ...
    @Inject
    WorkflowRunStore storeInterface;

    @Test
    void interfaceResolvesToJpaBean() {

        assertSame(store, storeInterface,
                "WorkflowRunStore interface should resolve to JpaWfRunStore when backend property is missing");
    }
    ...
    void taskLifecycleRoundTrips() {
        ...
        assertEquals(WorkflowRunStatus.COMPLETED, fetched.status());
    ...
    void stepWithRunAndTurnAndToolResultReconstitutes() {
        ...
        var req = new LlmRequest("system", null, "hello", List.of(), 0, null, null, null, null, java.util.List.of());
        ...
        store.stepCompleted(taskId, stepId, WorkflowRunStatus.COMPLETED, "step output");
        store.taskCompleted(taskId, WorkflowRunStatus.COMPLETED);
        ...
        assertEquals(WorkflowRunStatus.COMPLETED, step.status());
